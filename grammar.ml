...
(* Sets of nonterminals, used only below. *)
...
module Terminal = struct
  ...
  (* Determine how many terminals we have and build mappings
     ...
     is created. A new terminal "error" is created.
...
module Production = struct
  ...
  (* Create an array of productions. Record which productions are
     ...
     is created for every start symbol S. *)
  ...
  let start = StringSet.fold (fun nonterminal k ->
...
module ProductionMap = Patricia.Big
