...
(* TEMPORARY No GADTs for now. I represent the stack using nested
   anonymous tuples -- and some magic. *)

(* TEMPORARY lorsqu'on a plusieurs points d'entrée, le type de retour
   de toutes les fonctions dépend du point d'entrée choisi...  Avec
   GADTs, on doit pouvoir s'en tirer en paramétrant le type des états
   par un nouveau paramètre, qui indique le type du résultat final
   attendu. Chaque état initial fixera ce paramètre à la valeur
   souhaitée; et les autres états seront neutres vis-à-vis de ce
   paramètre, mais indiqueront qu'il reste constant tout au long de la
   pile. Le type de run doit alors être de la forme
     ('stack, 'result) state -> 'stack -> 'result
*)

(* TEMPORARY À terme, supprimer les assert dans le code engendré. *)
...
(* The variable that holds the stack. *)
...
(* A variable used to hold the lookahead token. *)
...
(* This inverts a mapping of tokens to productions into a mapping of
   productions to sets of tokens. This is needed to avoid producing
   two (or more) separate branches that call the same [reduce]
   function. Instead, we generate just one branch, guarded by a [POr]
   pattern. *)
...
(* Here is how we check whether state [s] should have a default
   ...
   reduction. This is explained in [Invariant]. Actually, we allow one
...
let (has_default_reduction : Lr1.node -> (Production.index * TerminalSet.t) option), hdrcount =
  Misc.tabulateo Lr1.number Lr1.fold Lr1.n (fun s ->
    ...
    match ProductionMap.is_singleton (invert (Lr1.reductions s)) with
	...
	if TerminalSet.mem Terminal.sharp toks then
	  ...
	  reduction
	else if Invariant.recoverer s then
...
(* Determine whether the [goto] function for nonterminal [nt] will push
   ...
   [goto] in turn transfer control to multiple [run] functions. Hence,
...
let gotopushes : Nonterminal.t -> bool =
  Nonterminal.tabulate (fun nt ->
	...
	accu &&
	match has_default_reduction target with
	| Some (prod, _) ->
...
(* These return either the empty list or the singleton list [[x]],
   depending on whether the symbol at hand carries a semantic value. *)
...
let (shiftreduce : Production.index -> bool), shiftreducecount =
  Production.tabulate (fun prod ->
    ...
    Invariant.fold_reduced (fun s accu ->
      accu && (match has_default_reduction s with None -> false | Some _ -> true)
           && (runpushes s)
    ) prod true
...
let envtypedef = {
  ...
  typerhs =
    TDefRecord ([
	 ...
	 head of the token streamm unless the flag [token_is_error] is
	 set. *)
...
(* A pattern that matches a token or a set set of tokens. *)

let tokpat tok =
  PData (tokenprefix (Terminal.print tok), tokval tok PWildcard)
...
(* Code for shifting from state [s] to state [s'] via the token [tok].
   ...
   off the input stream. It is also responsible for pushing a new
   stack cell. The rationale behind this decision is that there may be
   multiple shift transitions into [s'], so we actually share that
   code by placing it inside [run s'] rather than inside every
   transition. *)
...
let rec runactiondef s : valdef list =
  ...
  | None ->
	...
	) (invert reductions) (TerminalSet.empty, [])
      in
	...
	EMatch (EVar token,
	  Terminal.fold (fun tok branches ->
	    if Terminal.pseudo tok then
	      ...
	      { branchpat = tokpat tok;
		branchbody = EStringConst (Terminal.print tok) } :: branches
	  ) []
	)
      ),
      type2scheme (arrow ttoken tstring)
    )
}
...
(* This global variable holds an instance of the [Error]
   exception. It is preallocated in order to save code. *)

let excvaldef = {
  valpublic = false;
  valpat = PVar parse_error;
  valval = EData (excname, [])
} 
